...
void client_event_cb(evutil_socket_t fd, short evflags, void *args) {
  ...
  sleep(1);
  event_del(wt->event_);
 // wt->event_ = event_new(wt->eb, fd, EV_READ | EV_PERSIST | EV_ET, client_event_cb, wt);
...
void master_event_cb(evutil_socket_t fd, short evflags, void *args) {
  ...
  struct event* event_ = event_new(wt->eb, client_fd, EV_READ | EV_PERSIST, client_event_cb, wt);
  event_add(event_, 0);
...
WorkerThread::WorkerThread() {
  ...
  recv_fd_ = fds[0];
  eb = event_base_new();
  event_ = event_new(eb, recv_fd_, EV_READ | EV_PERSIST,
